...
public class EclipseUtilitiesTest extends TestCase
{
   ...
   public void testGetOutputFolder () throws CoreException, IOException
   {
      ...
      assertNotNull(f);
      File required = new File("junit-workbench-workspace/simpleproject/"
         + "build/classes/");
   ...
   public void testGetOutputFile () throws CoreException, IOException
   {
      ...
      assertNotNull(f);
      File required = new File("junit-workbench-workspace/simpleproject/"
         + "build/classes/" + "package1/package2/Class1.class");
      // compare the canonical files
      assertEquals(required.getCanonicalFile(), f.getCanonicalFile());

      f = EclipseUtilities.getOutputFile(cu, ".leJOS");
      assertNotNull(f);
      required = new File("junit-workbench-workspace/simpleproject/"
         + "build/classes/" + "package1/package2/Class1.leJOS");
      // compare the canonical files
      assertEquals(required.getCanonicalFile(), f.getCanonicalFile());
      ...
      assertNotNull(f);
      required = new File("junit-workbench-workspace/simpleproject/"
         + "build/classes/" + "package1/package2/Class1.leJOS.signature");
      // compare the canonical files
      assertEquals(required.getCanonicalFile(), f.getCanonicalFile());
   }

   public void testGetAbsoluteLocationForResource () throws CoreException,
      IOException
   {
      IProject project = sjp.getJavaProject().getProject();
      // the resource is named including the project name
      IPath path = new Path("/simpleproject/src/package1/package2/Class1.java");

      File f = EclipseUtilities.getAbsoluteLocationForResource(project, path);
      File required = new File("junit-workbench-workspace/simpleproject/"
         + "src/" + "package1/package2/Class1.java");
      // compare the canonical files
      assertEquals(required.getCanonicalFile(), f.getCanonicalFile());
   }
   ...
   public void testCollectLinkClasses () throws JavaModelException
   {
      ...
      cus = EclipseUtilities.collectLinkClasses(sjp
         .getPackage1Package2Class1CU());
      assertNotNull(cus);
      assertEquals(1, cus.length);
      // for Class2, we expect 0 CU
      cus = EclipseUtilities.collectLinkClasses(sjp
         .getPackage1Package2Class2CU());
      assertNotNull(cus);

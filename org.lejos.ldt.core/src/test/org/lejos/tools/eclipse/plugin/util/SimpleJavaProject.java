...
public class SimpleJavaProject extends AbstractJavaTestProject {
    ...
    public void createProject() throws CoreException, IOException,
        ...
        this.javaProject.setOption(JavaCore.COMPILER_CODEGEN_TARGET_PLATFORM,
        ...
        buf.append("  }\n");
        buf.append("}\n");
        this.package1Class1 = createType(this.package1, "Class1.java", buf
                .toString());
        ...
        buf.append("  }\n");
        buf.append("}\n");
        this.package1package2Class1 = createType(this.package1package2,
                "Class1.java", buf.toString());

        buf = new StringBuffer();
        buf.append("public class Class2 {\n");
        buf.append("}\n");

        this.package1package2Class2 = createType(this.package1package2,
                "Class2.java", buf.toString());
        // now enforce building
        setAutoBuilding(true);

        // now check, if Class1.java really exists
        File sourceFile = EclipseUtilities.getAbsoluteLocationForResource(this
                .getPackage1Package2Class1CU().getJavaProject().getProject(),
                this.getPackage1Package2Class1CU().getPath());
        if (!sourceFile.exists()) {
            IStatus status = new Status(Status.ERROR,
                    "org.lejos.tools.eclipse.plugin", 1, "source file "
                            + sourceFile + " does not exist", null);
        ...
        File classFile = EclipseUtilities.getOutputFile(this

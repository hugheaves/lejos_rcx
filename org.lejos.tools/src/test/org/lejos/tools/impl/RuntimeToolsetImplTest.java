...
public class RuntimeToolsetImplTest extends TestCase {
	...
	public static void main(String[] args) {
		junit.textui.TestRunner.run(
			new TestSuite(RuntimeToolsetImplTest.class));
	}
	...
	public void _testLinkValidFiles() throws ToolsetException, IOException {
		...
		toolset.link(
			outputFile,
			IRuntimeToolset.LINK_METHOD_ALL,
			true,
			classpath,
			new String[] { "package1.package2.Class1" },
			null);
	...
	public void testLinkInvalidFiles() throws ToolsetException {
		...
		try {
			toolset.link(
				new File("C:\\output_blabla\\blabla.leJOS"),
				IRuntimeToolset.LINK_METHOD_ALL,
				true,
				null,
				new String[] { new String("input_blabla.Blbalba")},
				null);
			fail("Oops, exception expected");
		} catch (ToolsetException ex) {
			// will be expected

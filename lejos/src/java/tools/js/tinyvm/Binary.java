...
public class Binary {
	...
	final RecordTable iStaticFields = new RecordTable("static fields", true,
			false);
	final RecordTable iConstantTable = new RecordTable("constants", false,
			false);
	...
	final RecordTable iConstantValues = new RecordTable("constant values",
			true, false);
	...
	public void processClasses(String[] entryClassNames, ClassPath classPath)
		...
		for (int pIndex = 0; pIndex < pSize; pIndex++) {
			...
			for (int i = 0; i < pInterfaceMethods.size(); i++) {
				classRecord.addUsedMethod((String) pInterfaceMethods
						.elementAt(i));
			}
	...
	public void processSpecialSignatures() {
		for (int i = 0; i < SpecialSignatureConstants.SIGNATURES.length; i++) {
			Signature pSig = new Signature(
					SpecialSignatureConstants.SIGNATURES[i]);
			iSignatures.addElement(pSig);
			iSpecialSignatures.put(pSig,
					SpecialSignatureConstants.SIGNATURES[i]);
		}
	}
	...
	public void processMethods(boolean iAll) throws TinyVMException {
		...
		for (int pIndex = 0; pIndex < pSize; pIndex++) {
			ClassRecord classRecord = (ClassRecord) iClassTable.get(pIndex);
			classRecord.storeMethods(iMethodTables, iExceptionTables,
					iSignatures, iAll);
		}
	}
	...
	public void log(ToolProgressMonitor monitor) throws TinyVMException {
		...
		monitor.log("Class records : " + iClassTable.size() + " ("
				+ iClassTable.getLength() + " bytes).");
		monitor.log("Field records : " + getTotalNumInstanceFields() + " ("
				+ iInstanceFieldTables.getLength() + " bytes).");
		monitor.log("Method records: " + getTotalNumMethods() + " ("
				+ iMethodTables.getLength() + " bytes).");
		monitor.log("Code          : " + iCodeSequences.size() + " ("
